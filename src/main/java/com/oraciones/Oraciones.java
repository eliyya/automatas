    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            try (var file = new Scanner(new File("./verbos.txt"))) {
                while (file.hasNext()) verbos.add(file.nextLine());
            }
        ...
        for (var i = 0; i < tokens.length; i++) {
            if (!verbos.contains(tokens[i])) continue;
        ...
        for (var i = 0; i < sujeto.length; i++) sujeto[i] = tokens[i];
        System.out.println("Sujeto: " + String.join(" ", sujeto));

        // Creo el predicado
        String[] predicado = new String[tokens.length - sujeto.length];
        for (var i = posicionVerbo; i < tokens.length; i++) predicado[i - posicionVerbo] = tokens[i];
